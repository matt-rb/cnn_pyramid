function [testPool,trainPool,indTest,indTrain] = Splitting_ComputeFFT(data,test_train_idxs,options)
...
trainPool= zeros(sum(pool1),options.legcnn*options.pyaramidlevel*2);
indTrain = zeros(sum(pool1),2);
for i=1:length(train_idxs)

    data1 = data{train_idxs(i,1)}{train_idxs(i,2)};
    [cnn_feature_video] = CnnDescriptor_FFT(data1,options);
    % [feature_pca] = MyPCA(cnn_feature_pyramid1',options);
...
%----------------------
S1 =0;%start fill big matrix
testPool= zeros(sum(pool2),options.legcnn*options.pyaramidlevel*2);
